...
export function TicketHistory({ className = '' }: TicketHistoryProps) {
  ...
  const fetchTickets = async (page = 1, search = '', status = 'all') => {
    try {
      ...
      const token = await import('@/utils/tokenManager').then(m => m.tokenManager.getValidToken());
      ...
      console.log('🎫 First ticket data:', data.data.tickets?.[0]);
      console.log('🎫 First ticket file_url:', data.data.tickets?.[0]?.file_url);
      console.log('🎫 All tickets file_urls:', data.data.tickets?.map(t => ({ id: t.id, file_url: t.file_url })));
  ...
  const handleViewImage = (ticket: Ticket) => {
    console.log('🖼️ Viewing image for ticket:', ticket);
    console.log('🖼️ File URL:', ticket.file_url);
    if (ticket.file_url) {
      setSelectedImageUrl(ticket.file_url);
      setImageModalOpen(true);
      setDetailsModalOpen(false); // Close details modal if open
    } else {
      console.warn('⚠️ No file_url found for ticket:', ticket.id);
      // Show error message to user
      alert('Image not available for this ticket. The file may not have been properly uploaded or the database needs to be updated.');
    }
  };
  ...
  const formatTicketDate = (dateString: string) => {
    try {
      ...
      // Return in MM/DD/YYYY format for display
